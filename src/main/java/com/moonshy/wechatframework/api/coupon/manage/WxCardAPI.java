package com.moonshy.wechatframework.api.coupon.manage;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.moonshy.wechatframework.api.coupon.manage.model.*;
import com.moonshy.wechatframework.api.coupon.manage.model.BatchGetCardRtnInfo;
import com.moonshy.wechatframework.api.coupon.manage.model.CardUpdate;
import com.moonshy.wechatframework.api.coupon.manage.model.DelRtnInfo;
import com.moonshy.wechatframework.api.coupon.manage.model.GetCardDetailRtnInfo;
import com.moonshy.wechatframework.api.coupon.manage.model.GetCardRtnInfo;
...
public class WxCardAPI {
	...
	public static DelRtnInfo doDelCard(String newAccessToken, String card_id) {
		if (newAccessToken != null) {
			...
			DelRtnInfo delRtnInfo = JSON.parseObject(result, DelRtnInfo.class);
			return delRtnInfo;
		}
		return null;
	}
	...
	public static GetCardRtnInfo doGetCard(String newAccessToken, String code, String card_id) {
		if (newAccessToken != null) {
			...
			String result = HttpUtils.post(requestUrl, json);
			GetCardRtnInfo getCardRtnInfo = JSON.parseObject(result, GetCardRtnInfo.class);
	...
	public static BatchGetCardRtnInfo doBatchGetCard(String newAccessToken, int offset, int count) {
		if (newAccessToken != null) {
			...
			String result = HttpUtils.post(requestUrl, json);
			BatchGetCardRtnInfo batchGetCardRtnInfo = JSON.parseObject(result, BatchGetCardRtnInfo.class);
	...
	public static GetCardDetailRtnInfo doGetCardDetail(String newAccessToken, String card_id) {
		if (newAccessToken != null) {
			...
			String json = "{\"card_id\":\"" + card_id + "\"}";
			String result = HttpUtils.post(requestUrl, json);
			GetCardDetailRtnInfo getCardDetailRtnInfo = JSON.parseObject(result, GetCardDetailRtnInfo.class);
	 ...
	 * 注：为避免用户疑惑，建议仅在发生转赠行为后（发生转赠后，微信会通过事件推送的方 式告知商户被转赠的卡券code）对用户的code进行更改。
	 * 
	 * @param code
	 *            卡券的code 编码。
	 * @param card_id
	...
	public static CommCardRtnInfo doUpdateCode(String newAccessToken, String code, String card_id, String new_code) {
		if (newAccessToken != null) {
			...
			String result = HttpUtils.post(requestUrl, json);
			CommCardRtnInfo commCardRtnInfo = JSON.parseObject(result, CommCardRtnInfo.class);
	...
	public static CommCardRtnInfo doUpdateCard(String newAccessToken, CardUpdate cardUpdate) {
		if (newAccessToken != null) {
			...
			String result = HttpUtils.post(requestUrl, JSONObject.toJSONString(cardUpdate));
			CommCardRtnInfo commCardRtnInfo = JSON.parseObject(result, CommCardRtnInfo.class);
			return commCardRtnInfo;
		}
		return null;
