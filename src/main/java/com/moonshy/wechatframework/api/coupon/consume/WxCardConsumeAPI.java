package com.moonshy.wechatframework.api.coupon.consume;

import com.alibaba.fastjson.JSON;
import com.moonshy.wechatframework.api.coupon.consume.model.ConsumeRtnInfo;
import com.moonshy.wechatframework.api.coupon.consume.model.EncryptRtnInfo;
...
public class WxCardConsumeAPI {
	...
	public static ConsumeRtnInfo doConsume(String newAccessToken, String code, String card_id) {
		if (newAccessToken != null) {
			String requestUrl = consume_code_url.replace("ACCESS_TOKEN", newAccessToken);
			String json = emptyStrJson("code", code, "card_id", card_id);
			String result = HttpUtils.post(requestUrl, json);
			ConsumeRtnInfo consumeRtnInfo = JSON.parseObject(result, ConsumeRtnInfo.class);
	...
	public static EncryptRtnInfo doDecrypt(String newAccessToken, String encrypt_code) {
		if (newAccessToken != null) {
			String requestUrl = decrypt_code_url.replace("ACCESS_TOKEN", newAccessToken);
			String json = emptyStrJson("encrypt_code", encrypt_code);
			String result = HttpUtils.post(requestUrl, json);
			EncryptRtnInfo encryptRtnInfo = JSON.parseObject(result, EncryptRtnInfo.class);
