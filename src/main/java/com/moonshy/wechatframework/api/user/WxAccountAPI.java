...
import com.moonshy.wechatframework.api.user.model.Qrcode;
import com.moonshy.wechatframework.api.user.model.QrcodeType;
...
public class WxAccountAPI {
	Logger logger = LoggerFactory.getLogger(WxAccountAPI.class);
	 ...
	 * @return
	 */
	public String shortUrl(String longUrl, String access_token) {
		JSONObject jsonObject = new JSONObject();
		jsonObject.put("action", "long2short");
		jsonObject.put("long_url", longUrl);
		String requestData = jsonObject.toString();
		logger.info("request data " + requestData);
		String resultStr = HttpUtils.post(SHORTURL_POST_URL + access_token, requestData);
	 ...
	 *
	 * @param sceneId
	 *            场景值ID,目前参数只支持1--100000
	 * @return
	 */
	public Qrcode createQrcodePerpetual(long sceneId, String access_token) {
		return createQrcodeTicket(QrcodeType.QR_LIMIT_SCENE, null, sceneId, null, access_token);
	}
	 ...
	 * @return
	 */
	public Qrcode createQrcodePerpetualstr(String sceneStr, String access_token) {
		return createQrcodeTicket(QrcodeType.QR_LIMIT_STR_SCENE, null, null, sceneStr, access_token);
	}
	 ...
	 * @return
	 */
	public Qrcode createQrcodeTemporary(long sceneId, int expireSeconds, String access_token) {
		return createQrcodeTicket(QrcodeType.QR_SCENE, expireSeconds, sceneId, null, access_token);
	}

	private Qrcode createQrcodeTicket(QrcodeType qrcodeType, Integer expireSeconds, Long sceneId, String sceneStr, String access_token) {
		...
		logger.info("request data " + requestData);
		String resultStr = HttpUtils.post(QRCODE_POST_URL + access_token, requestData);
		logger.info("return data " + resultStr);
		try {
			WeChatUtil.isSuccess(resultStr);
		} catch (WeChatException e) {
